/**
 * Translate German lessons from English overlay â†’ Turkish (tr-TR).
...
const LANG = 'fa'
const LANG_FULL = 'Persian/Farsi (Iran, fa-IR)'
...
Strict rules:
  â€¢ German vocabulary and example sentences stay UNCHANGED â€” they are the language being taught.
  â€¢ Keep "german" and "example" fields in vocabulary unchanged.
  â€¢ Translate "arabic" fields in vocabulary to ${LANG_FULL} (the field is named "arabic" but holds the gloss in the target language).
  â€¢ Translate "exampleArabic" fields to ${LANG_FULL}.
  â€¢ In grammar tables: translate non-German cells; keep German cells unchanged.
  â€¢ In rules: translate "rule" and "translation"; keep "example" (German) unchanged.
  â€¢ In examples (format "German sentence â€” English sentence"): keep the German half; replace the English half with ${LANG_FULL}.
  â€¢ In exercises: translate question text and hints; keep answers in German.
...
Return ONLY a JSON object â€” a Partial<Lesson> overlay with key "${LANG}". NO commentary:
{
  "${LANG}": {
    "title": "...",
    "grammar": {
...
async function main() {
  for (const level of LEVELS) {
    ...
    if (!fs.existsSync(enOverlayPath)) {
      console.log(`Skipping ${level} â€” no en overlay found`)
      continue
    }
