/**
 * Fix: re-translate tr.writingExercise by splitting into individual sub-keys.
 */
import fs from 'fs'
...
let tr = JSON.parse(fs.readFileSync(path.join(ROOT, 'messages/tr.json'), 'utf8'))
...
async function translateOne(key, value) {
  ...
  const prompt = `Translate this UI strings JSON section from English to Turkish (Turkey, tr-TR) for GoGermany (website helping people move to Germany).

OUTPUT RULES (CRITICAL):
- Your response must be ONLY valid JSON â€” start with { and end with }
- No markdown fences, no preamble, no explanation
- Translate ALL string values to natural Turkish
...
async function main() {
  if (!tr.writingExercise) tr.writingExercise = {}
  ...
  for (const subKey of subKeys) {
    ...
    const ruVal = tr.writingExercise[subKey]
    ...
    try {
      ...
      tr.writingExercise[subKey] = typeof enVal === 'object' ? translated : translated.value ?? translated
      fs.writeFileSync(path.join(ROOT, 'messages/tr.json'), JSON.stringify(tr, null, 2), 'utf8')
      console.log('âœ“')
